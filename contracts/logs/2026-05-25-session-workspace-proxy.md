...
- `backend/cmd/server/main.go` — 注册 `/api/session/:sessionId/{files/*,diff,commit,revert}` 路由组
...
- **Backend**：新增 `/api/session/{sessionId}/` 下 files/diff/commit/revert 代理路由，内部解析 sessionId → workspaceId 后转发至 AgentEnd，不修改现有 handler
