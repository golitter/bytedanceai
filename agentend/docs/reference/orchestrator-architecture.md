...
**动态上下文计算**：Pin 约束和编排经验在 `skill_prepare_node` 中单独计算，存入 `state["pin_context"]` 和 `state["evolution_context"]`，由 `reason_node` 以消息列表方式注入。
...
| `pin_context` | `str` | Pin 约束文本（skill_prepare 计算） |
...
- LLM 自动生成 1-3 句摘要
- 注入方式：`skill_prepare_node` 计算后存入 `state["pin_context"]`，`reason_node` 以 `SystemMessage` 注入消息列表
