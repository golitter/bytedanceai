...
Orchestrator 只做了「拆解 + 写文件」，没有：
- **调度**：不把任务分发给对应 Agent
- **跟踪**：不知道任务是否被执行、执行到哪
- **回调**：Agent 完成任务后无法通知 Orchestrator
- **重规划**：如果某个任务失败，无法重新拆解
...
- **LLM 可能在 JSON 前后加解释文字** — 如果没加代码块包裹，直接 `json.loads(text)` 会崩溃
- **LLM 可能在 JSON 内部加注释** — 标准 JSON 不支持注释，但 LLM 经常加
- **没有 fallback** — 一次解析失败，整个规划就失败

`_extract_json` 的异常**完全未捕获**，会直接以 `json.JSONDecodeError` 冒泡到 API 层，变成 500 错误。
...
整个 `orchestrator/` 目录下没有一行 `logger` 调用。生产环境中失败时完全没有可观测性。
...
│             │ • 只规划不执行（无调度/跟踪/回调/重规划）          │
│             │ • 与 Workspace 体系完全割裂                       │
├─────────────┼──────────────────────────────────────────────────┤
│ 可靠性      │ • JSON 提取脆弱，无 fallback                      │
│             │ • 每次调用新建 LLM 实例                           │
│             │ • 同步文件 I/O 阻塞事件循环                       │
│             │ • assert 做控制流                                 │
│             │ • 无重试机制                                      │
...
| **P1** | JSON 解析无错误处理 | 生产环境必现的 500 错误 |
