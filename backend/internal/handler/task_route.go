...
func resolveMessageRoute(req RunTaskReq, sessions []model.Session) (MessageRoute, error) {
	...
	if req.SkipUserMessage {
		if s, ok := findSessionByID(sessions, req.SessionID); ok {
			route.AgentName = s.AgentName
			route.RouteID = buildRouteAgentForSession(sessions, s.SessionID).RouteID
		}
		return route, nil
	}
...
func requestedAgentType(req RunTaskReq, sessions []model.Session) string {
	if req.AgentType != "" {
		return req.AgentType
	}
	if s, ok := findSessionByID(sessions, req.SessionID); ok && s.AgentType != "" {
		return s.AgentType
	}
	return "claude-code"
}
...
func buildRouteAgents(sessions []model.Session) []routeAgent {
	...
	for _, s := range sessions {
		base := strings.TrimSpace(s.AgentName)
		if base == "" {
			base = strings.TrimSpace(s.AgentType)
		}
		if base == "" {
			base = s.SessionID
		}
		...
		agents = append(agents, routeAgent{
			...
			Aliases:      uniqueNonEmpty(append([]string{routeID, s.AgentName, s.AgentType}, agentTypeAliases[s.AgentType]...)...),
