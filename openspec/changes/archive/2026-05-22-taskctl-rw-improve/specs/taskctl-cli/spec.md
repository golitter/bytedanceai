...
`parsePath` SHALL return only `sessionID` and `sharedDir`. It SHALL NOT return `taskID`.

#### Scenario: parsePath signature
- **WHEN** `parsePath` is called
- **THEN** it SHALL return `(sessionID, sharedDir string, err error)` with exactly three return values
