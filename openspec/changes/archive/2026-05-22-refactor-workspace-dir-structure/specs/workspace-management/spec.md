...
系统 SHALL 定义 `Workspace` 数据类，包含字段：`id`（str）、`task_id`（str）、`agent_name`（str，存储 agent 类型标识如 "claude-code"）、`agent_type`（AgentType 枚举）、`repo_path`（str）、`worktree_path`（str）、`branch_name`（str）、`session_id`（str，必传）、`status`（WorkspaceStatus 枚举）、`created_at`（datetime）、`container_id`（可选 str，默认 None）。

`__post_init__` SHALL 使用 `session_id`（而非 `agent_name`）生成 `worktree_path` 和 `branch_name`。

#### Scenario: Create workspace instance with session_id
- **WHEN** 创建 `Workspace(task_id="task-123", agent_name="claude-code", agent_type=AgentType.CLAUDE_CODE, session_id="sess-abc", repo_path="/repos/project")`
- **THEN** `branch_name` SHALL 自动生成为 `"agent/sess-abc/task-123"`，`worktree_path` SHALL 为 `/repos/worktrees/task-123/sess-abc`
...
`WorkspaceManager` SHALL 提供 `create(repo_path, task_id, agent_name, session_id, agent_type)` 方法，自动创建两级分支和 git worktree。MUST 先创建 `task/{task_id}` 集成分支（从 main），再创建 `agent/{session_id}/{task_id}`（从 task branch）。

#### Scenario: Create workspace with new task branch
- **WHEN** 调用 `manager.create(repo_path="/repos/project", task_id="task-123", agent_name="claude-code", session_id="sess-abc")` 且 task branch 不存在
- **THEN** SHALL 先创建 branch `task/task-123`（from main），再创建 branch `agent/sess-abc/task-123`（from task/task-123），创建 worktree 目录

#### Scenario: Create workspace with existing task branch
- **WHEN** 调用 `manager.create(...)` 且 `task/task-123` 已存在
- **THEN** SHALL 直接从 `task/task-123` 创建 `agent/{session_id}/{task_id}`，不重复创建 task branch

#### Scenario: Create multiple workspaces for same task with different sessions
- **WHEN** 同一 task_id 创建 session_id="sess-1" 和 session_id="sess-2" 两个 workspace
...
- **WHEN** 两个请求同时为同一 task_id 创建不同 session workspace
