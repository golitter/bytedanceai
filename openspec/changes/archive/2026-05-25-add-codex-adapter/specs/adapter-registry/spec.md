...
The AdapterRegistry SHALL include OrchestratorAdapter registered under `AgentType.ORCHESTRATOR` (value `"orchestrator"`). It SHALL also include CodexAdapter registered under `AgentType.CODEX` (value `"codex"`). Both SHALL be instantiatable via `registry.get()` just like existing adapters.
