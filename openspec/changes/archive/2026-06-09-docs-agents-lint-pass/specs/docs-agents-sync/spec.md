...
AGENTS.md 中描述的 `make` 命令 MUST 与根 [Makefile](Makefile) 中的实际 target 一致；新增 target MUST 补充，已废弃 target MUST 移除。
...
- **THEN** 该端点在 [backend](backend/) 路由注册中存在且方法一致
...
- **THEN** 该 change 在归档前补齐对应 `NN-<module>.md`，且符合上述格式
...
- **THEN** 标记关联的事项要么已完成（移除 TODO），要么仍有意义（保留并在本 change 范围内不动）

### Requirement: 文档 lint pass SHALL 严格不动代码与配置

本次 change MUST NOT 修改 `.go` / `.py` / `.ts` / `go.mod` / `package.json` / `pyproject.toml` / `Makefile` / `.env*` 等代码、依赖、构建、配置文件。`AGENTS.md`（任何端）不可作为 lint 规则的输入被 lint 自身修改 — 只能按 skill 规则被 AGENTS.md 类专项修改。

#### Scenario: 仅文档被修改
- **WHEN** `git diff --name-only HEAD~1..HEAD` 查看本次 change 的提交
