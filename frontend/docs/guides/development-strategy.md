...
Store 按领域拆分，不合并成一个大 store。用 create 创建，组件通过 selector 订阅具体字段，避免不必要的 re-render。当前有 chat store（导航 + 会话消息状态 + 多 Agent runtime blocks）和 admin store（管理面板认证 + 菜单状态），后续按需增加 user store、ui store 等。
